...
async function loadOriginalReportData(
  ...
  // Verified, completed live OT is not a short/over exception in the LINE report.
  // Fail closed if OT verification fails; never send a misleading report.
  const {data:completedOt,error:otError}=await supabase.rpc("kitty_live_line_ot_complete",{report_date:date});
  if(otError || !Array.isArray(completedOt)) throw otError || new Error("OT_REPORT_UNAVAILABLE");
  const completedOtEmployees=new Set(completedOt);
  ...
  for (const row of dailyRows || []) {
    ...
    if (
      !completedOtEmployees.has(employee.id) &&
      Math.round(item.short_hours * 60) > 0 &&
      row.last_out_at
    ) {
