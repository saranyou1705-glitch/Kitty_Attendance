...
create or replace function public.kitty_staging_personnel_v1(actor text,operation text,payload jsonb default '{}')
...
create table kitty_staging.employee_contacts(
 employee_id uuid primary key,phone text not null default '',email text not null default '',
 version integer not null default 1,updated_at timestamptz not null default now()
);
alter table kitty_staging.employee_contacts enable row level security;
revoke all on kitty_staging.employee_contacts from public,anon,authenticated;
create function public.kitty_staging_self_profile_v1(actor text,operation text,payload jsonb default '{}')
returns jsonb language plpgsql security definer set search_path=pg_catalog as $f$
declare e public.employees%rowtype;p kitty_staging.personnel%rowtype;c kitty_staging.employee_contacts%rowtype;
 phone_value text;email_value text;days jsonb;
begin
 if actor is null or actor='' then raise exception 'UNAUTHENTICATED';end if;
 if (select count(*) from public.employees where line_user_id=actor and active)<>1 then raise exception 'FORBIDDEN';end if;
 select * into e from public.employees where line_user_id=actor and active;
 ...
 select * into c from kitty_staging.employee_contacts where employee_id=e.id;
 if operation='save' then
   ...
   insert into kitty_staging.employee_contacts(employee_id,phone,email) values(e.id,phone_value,email_value)
   on conflict(employee_id) do update set phone=excluded.phone,email=excluded.email,version=kitty_staging.employee_contacts.version+1,updated_at=now() returning * into c;
 end if;
 select * into p from kitty_staging.personnel where employee_id=e.id;
 select coalesce(jsonb_agg((array['MON','TUE','WED','THU','FRI','SAT','SUN'])[iso_dow] order by iso_dow),'[]') into days
 from public.employee_weekly_dayoffs where employee_id=e.id and active and iso_dow between 1 and 7;
...
revoke all on function public.kitty_staging_self_profile_v1(text,text,jsonb) from public,anon,authenticated;
grant execute on function public.kitty_staging_self_profile_v1(text,text,jsonb) to service_role;
