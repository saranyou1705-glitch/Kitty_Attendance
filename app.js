...
async function ownOtStatus(date,info={},daily){
...
async function requestsView(kind){
 ...
 return panel(`<div class="section-heading"><h2>${kind==='leave'?'คำขอลา':'คำขอลงเวลา'}</h2><button class="btn secondary" data-action="retry">โหลดใหม่</button></div>${requestList(filtered)}<p class="panel-sub">ล่าสุดไม่เกิน 1,000 รายการ${CONFIG.requestsLive?' · คำขอมีผลกับสรุปเวลาจริง':' · ชุดทดลอง ไม่กระทบระบบเดิม'}</p>`)+(CONFIG.requestsLive?await managerRequestHistory(kind):'')+(kind==='leave'?`<details class="panel"><summary>การลาที่บันทึกแล้ว</summary>${await leaveManagementView()}</details>`:'');
}
async function managerRequestHistory(kind){
 try{
